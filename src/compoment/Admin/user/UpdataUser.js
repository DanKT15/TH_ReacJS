...
export default function UpdataUser() {
    ...
    const HandleSubmit = async (e) => {
        ...
        const Updatauser = await API_User.edit_user(datauUser.username, datauUser.password, datauUser.fullname, datauUser.address, datauUser.sex, datauUser.email, datauUser.groupid);
    ...
    useEffect( () => {
        ...
        const importdata = async () => {
            ...
            const oldataUser = await API_User.detail_user(username);

            const userinfo = oldataUser.data.list[0];

            setUser({
                ...datauUser, username: userinfo.username, password: userinfo.password, fullname: userinfo.fullname, address: userinfo.address, sex: userinfo.sex, email: userinfo.email, groupid: userinfo.groupid
            });
